  ...
  for (const orderedItem of orderedItems) {
    const itemIndex = cart.items.findIndex(
      (cartItem) =>
        ...
        String(cartItem.selectedSize || cartItem.size || "") === String(orderedItem.selectedSize || orderedItem.size || "")
    );
...
export const createOrder = async (req, res) => {
  ...
  for (const item of items) {
    ...
    if (isSizeReq) {
      if (!selectedSize) {
        return res.status(StatusCodes.BAD_REQUEST).json({ message: `Please select a size for ${product.name}.` });
      }
      if (Array.isArray(product.sizes) && product.sizes.length > 0 && !product.sizes.includes(selectedSize)) {
        return res.status(StatusCodes.BAD_REQUEST).json({ message: `Size ${selectedSize} is not available for ${product.name}.` });
      }
    }
    ...
    normalizedItems.push({
      ...
      selectedColor,
      selectedSize: isSizeReq ? selectedSize : "",
      size: isSizeReq ? selectedSize : "",
      sku:
        product.variants?.find((variant) => variant.color === selectedColor)?.sku ||
        `${String(product.slug || product._id)}-${selectedColor || "default"}${selectedSize ? `-${selectedSize}` : ""}`
    });
  }
  ...
  for (const item of order.items) {
    const existing = cart.items.find(
      (entry) =>
        ...
        String(entry.selectedSize || entry.size || "") === String(item.selectedSize || item.size || "")
    );
    if (existing) {
