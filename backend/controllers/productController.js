...
const formatProduct = (product) => {
  ...
  const rawSizes = parseArrayInput(doc.sizes);
  const sizeApplicable = Boolean(doc.sizeApplicable ?? (rawSizes.length > 0));
  const sizes = sizeApplicable ? rawSizes : [];
  ...
  return withPopularity({
    ...
    sizes,
    sizeApplicable,
    size_applicable: sizeApplicable,
...
const buildProductPayload = (body, previousProduct) => {
  ...
  const sizeApplicable = parseBoolean(
    body.sizeApplicable ?? body.size_applicable,
    previousProduct?.sizeApplicable ?? (parseArrayInput(body.sizes).length > 0)
  );
  const rawSizes = parseArrayInput(body.sizes ?? body.availableSizes ?? body.available_sizes);
  const sizes = sizeApplicable
    ? (rawSizes.length ? rawSizes : previousProduct?.sizes || [])
    : [];
