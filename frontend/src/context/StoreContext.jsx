...
export const StoreProvider = ({ children }) => {
  ...
  const addToCart = useCallback(async (product, qty = 1, selectedColor = "", selectedSize = "") => {
    ...
    const matcher = (item) =>
      ...
      String(item.selectedSize || item.size || "") === String(normalizedSize || "");
    ...
    showToast({
      title: "Added to cart",
      message: `${product.name}${normalizedSize ? ` (Size: ${normalizedSize})` : ""}${normalizedColor ? ` - ${normalizedColor}` : ""} is now in your bag.`,
      tone: "success"
    });
        ...
        .map((cartItem) => {
          const orderedItem = orderedItems.find(
            (item) =>
              ...
              String(item.selectedSize || item.size || "") === String(cartItem.selectedSize || cartItem.size || "")
          );
