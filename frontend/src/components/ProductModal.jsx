...
import { formatCurrency, getMarketPrice, getOfferPrice, getProductColors, getProductImage, getProductSizes, hasOfferPrice, isSizeApplicable, STANDARD_SIZES } from "../utils/catalog";
...
export default function ProductModal({ product, open, onClose }) {
  ...
  // Track active selected color & size
              ...
              <div className="flex flex-col justify-between">
                <div>
                  ...
                  {isSizeApplicable(product) && (
                    <div className={`mt-8 ${sizeError ? "rounded-2xl border-2 border-red-200 bg-red-50/40 p-4 transition-all" : ""}`}>
                      <div className="flex items-center justify-between">
                        <h3 className="text-xs font-black uppercase tracking-[0.2em] text-stone-700">
                          Select Size {selectedSize && <span className="font-black text-stone-900">({selectedSize})</span>}
                        </h3>
                        <button
                          type="button"
                          onClick={() => setSizeGuideOpen(true)}
                          className="flex items-center gap-1 text-xs font-bold text-brand-700 underline decoration-brand-300 underline-offset-4 hover:text-brand-900"
                        >
                      ...
                      <div className="mt-3 flex flex-wrap gap-2.5">
                        {(productSizes.length > 0 ? productSizes : STANDARD_SIZES).map((size) => (
                          <button
                            ...
                            className={`min-h-[2.5rem] min-w-[3rem] rounded-xl border-2 px-3.5 py-1.5 text-xs font-bold transition-all active:scale-95 ${
                              selectedSize === size
                      ...
                      {sizeError && <p className="mt-2 text-xs font-bold text-red-600">{sizeError}</p>}
                    </div>
                  )}
