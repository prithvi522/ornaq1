...
import { getProductColors, getProductImage, getProductSizes, isJewelleryCategory, mergeCategories, STANDARD_SIZES } from "../../utils/catalog";
...
export default function ProductEditor({ product, categories, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    const isApplicable = Boolean(product.sizeApplicable ?? (productSizes.length > 0));
    setForm({
      name: product.name || "",
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const colors = form.colorsInput
        ...
        .map((item) => item.trim())
        .filter(Boolean);
      const sizes = form.sizeApplicable
        ? form.sizesInput
            .split(",")
            .map((item) => item.trim())
            .filter(Boolean)
      ...
      await api.patch(`/products/${product._id}`, {
        ...form,
        sizeApplicable: Boolean(form.sizeApplicable),
        ...
        <form onSubmit={submit} className="grid gap-6 px-6 py-6 md:grid-cols-[1.2fr_0.8fr]">
          <div className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2 space-y-4 rounded-2xl border border-stone-200 bg-stone-50/60 p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <label className="text-xs font-bold uppercase tracking-[0.18em] text-stone-700">Size Applicable</label>
                    <p className="text-[11px] text-stone-400">Required for Kurtis, Dresses, Tops & Western Wear. Disable for Sarees & Jewellery.</p>
                  </div>
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => setForm((c) => ({ ...c, sizeApplicable: true }))}
                    ...
                    <button
                      type="button"
                      onClick={() => setForm((c) => ({ ...c, sizeApplicable: false, sizesInput: "" }))}
                      className={`rounded-xl px-4 py-2 text-xs font-black uppercase tracking-wider transition-all ${
                        !form.sizeApplicable
                          ? "bg-stone-900 text-white shadow-md"
                          : "bg-white text-stone-600 border border-stone-200 hover:bg-stone-50"
                  ...
                  <div className="border-t border-stone-200 pt-3">
                      ...
                      {STANDARD_SIZES.map((size) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex h-11 min-w-[3rem] px-3 items-center justify-center rounded-xl border text-sm font-bold transition-all active:scale-95 ${
                              isSelected
                                ? "border-brand-700 bg-brand-700 text-white shadow-md shadow-brand-200"
                    ...
                    {selectedSizes.length === 0 && (
                      <p className="mt-2 text-xs font-semibold text-amber-600">
                        Please select at least one available size for this product.
                      </p>
                    )}
                  </div>
                )}
